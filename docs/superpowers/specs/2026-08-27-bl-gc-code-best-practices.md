...
2. `$HOME` 전체를 `find ... -print0`로 스캔하고, Git의 `git-dir`/`git-common-dir` 관계로 linked worktree만 식별한다.
...
- `find` 권한 오류와 “home scan unavailable” 상태를 서로 구분하는 진단 메시지
...
- [`run_cleanup`](../../../users/shared/programs/baleen-toolkit.sh#L404-L414)은 각 외부 cleanup 명령을 `if "$@"; then ... else ... fi`로 감싸므로 개별 도구 실패를 기록하고 다음 단계로 진행한다.
...
- 직접 삭제는 [`clear_known_directory`](../../../users/shared/programs/baleen-toolkit.sh#L416-L430)의 allowlist를 통과한 두 경로에만 수행된다. 현재 코드에는 외부 입력을 wildcard로 붙여 `rm -rf`하는 패턴이 없다.
...
- 현재 작업 트리는 [`find "$HOME" ... -print0`](../../../users/shared/programs/baleen-toolkit.sh#L291-L329)로 home 아래 `.git` 파일을 찾는다.
- `.git` 디렉터리는 건너뛰므로 일반 저장소의 main worktree를 후보로 만들지 않는다.
- `git-dir`과 `git-common-dir`이 다른 경로인 항목만 linked worktree로 인정한다.
...
- 최종 삭제는 [`git worktree remove "$target"`](../../../users/shared/programs/baleen-toolkit.sh#L487-L503)이고 `--force`가 없다. inventory 시점과 삭제 시점 사이에 worktree가 dirty가 되어도 Git의 최종 clean 검사가 보호막으로 남는다.
- missing worktree나 깨진 `.git` marker는 `canonical_path` 또는 Git metadata 확인 단계에서 후보가 되지 않는다. 이는 자동 복구나 수동 삭제로 확장하지 않는 fail-closed 동작이다.

#### 현재 작업 트리의 NUL-safe home scan과 locked 보호

현재 작업 트리의 scanner는 `while IFS= read -r -d '' git_file`로 NUL-delimited `find` output을 읽는다. Git marker 경로에 개행이 포함돼도 경계를 보존한다.

`locked` worktree는 Git이 force 없이 제거하지 않는 보호 대상이므로 후보에서 먼저 제외한다. Git의 `prunable` metadata는 이번 marker scan에서 직접 정리 대상으로 다루지 않으며, 깨진 marker는 Git metadata 확인 단계에서 탈락한다.

남은 후속 방향은 `find`의 권한 오류를 별도 진단하고, Git metadata를 한 번에 읽어 중복 명령을 줄이는 것이다.
...
- 현재 script 본문은 `bash`를 외부 명령으로 호출하지 않고, `sed`도 사용하지 않는다. `find`는 home scan에 필요하므로 `findutils`를 runtimeInputs에 명시했다. `bash`와 `gnused`를 줄이는 것은 closure 정리 후보지만 기능 안전성과는 무관하다.
...
- [`docker_available`](../../../users/shared/programs/baleen-toolkit.sh#L352-L354)은 `docker info`로 daemon 연결을 확인하고, 불가능하면 Docker 단계를 건너뛴다. `docker info`는 Docker 공식 문서의 `docker system info` alias다. [Docker `system info`](https://docs.docker.com/reference/cli/docker/system/info/)
- 통계는 `docker system df`만 호출하고 삭제하지 않는다.
- cleanup은 [`clean_docker`](../../../users/shared/programs/baleen-toolkit.sh#L512-L516)의 세 명령으로 분리되어 있다.
...
- daemon이 probe 이후 사라져도 각 실제 명령은 [`run_cleanup`](../../../users/shared/programs/baleen-toolkit.sh#L404-L414)이 실패를 기록하고 다음 단계로 진행한다.
...
이 테스트는 핵심 worktree 정책의 좋은 tracer다. 이번 적용에서 portable `stat` fallback과 locked worktree 보존 회귀를 추가했고, NUL-safe home scan과 `bashOptions`도 source assertion으로 고정했다. 실제 built wrapper의 `--help`, `list`, `gc help`, `gc --dry-run`도 별도로 실행했다.

남은 우선순위 높은 테스트는 다음과 같다.

1. `du`와 `df`가 실패하는 fixture에서 `?`/`unavailable` 출력과 종료 상태를 검증한다.
2. 개행이 포함된 worktree 경로 fixture를 추가해 NUL-safe home scan을 동작 수준에서 검증한다.
...
2. GNU/BSD `stat` 순서를 고치고, `$HOME` 전체의 NUL-safe marker scan과 linked/locked 보호를 추가했다.
3. portable stat, home 전체 탐색, locked worktree에 대한 회귀 테스트를 추가했다.
4. `nix fmt`, focused Nix check, `make test-build`, `bash -n`, ShellCheck, pre-commit 전체 검사로 검증했다.

후속 범위는 실패하는 `du`/`df` fixture, built wrapper 전용 fixture, find 권한 오류의 세부 관찰성이다.
