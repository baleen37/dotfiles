...
**Direction check:** the test iterates `subs = cacheConfig.substituters` and asserts each is in the file. If we _remove_ a substituter from `cache-config.nix`, the test still passes (fewer needles). To trigger drift detection, we must instead _add_ a substituter to `cache-config.nix` that is not in the other files.
