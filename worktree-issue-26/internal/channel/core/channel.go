...
type ChannelSettings struct {
	MaxVideoDuration int    `json:"max_video_duration"` // Maximum video duration in seconds
	VideoWidth       int    `json:"video_width"`        // Video width in pixels
	VideoHeight      int    `json:"video_height"`       // Video height in pixels
	VideoFPS         int    `json:"video_fps"`          // Video frames per second
	Language         string `json:"language"`           // Primary language for content
}
