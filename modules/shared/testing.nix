  ...
  config = mkIf cfg.enable {
    ...
    _module.args.testing = {
        ...
        in
          ...
          then throw "Unsupported platform: ${platform}. Supported platforms: ${builtins.concatStringsSep ", " cfg.crossPlatform.targetPlatforms}"
            ...
            paths = {
              ...
              configDirectory = if hasInfix "darwin" platform 
                               then "$HOME/.config" 
                               else "$HOME/.config";
            };
            tools = {
      ...
      runPlatformTests = { tests, environment }:
        let
          platformTests = builtins.filter (test: 
      ...
      getCompatibilityMatrix = { ... }:
        {
          ...
          compatibility = builtins.listToAttrs (map (platform: {
            name = platform;
            value = {
              supported = true;
              features = [
      ...
      validatePlatformCapabilities = { platform, requiredCapabilities }:
        ...
        in builtins.all (cap: builtins.elem cap availableCapabilities) requiredCapabilities;
      
      applyPlatformConfig = { config, platform }:
        let
          platformConfig = if hasInfix "darwin" platform 
