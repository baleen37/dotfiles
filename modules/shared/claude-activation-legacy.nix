# Shared Claude Activation Library
#
# 이 파일은 Claude 활성화 스크립트를 생성하는 함수를 제공합니다.
# Home Manager 모듈이 아닌 라이브러리 함수로 사용됩니다.
#
# 사용법:
# - Platform home-manager.nix에서 import하여 활성화 스크립트 생성
# - 공통 로직을 중앙화하여 중복 제거
#
# 매개변수:
# - config: Home Manager config
# - lib: Nixpkgs lib
# - self: Flake self reference (optional)
...
let
  ...
  # 소스 디렉토리 해결 로직 (claude-activation.nix에서 통합)
  ...
  # Claude 활성화 스크립트 (claude-activation.nix 통합)
  claudeActivationScript = ''
    set -euo pipefail
