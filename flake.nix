  ...
  outputs =
    ...
    let
      ...
      pre-commit-check =
        ...
        git-hooks.lib.${system}.run {
          src = ./.;
          hooks = {
            nixfmt-rfc-style.enable = true; # Official Nix formatter
            deadnix.enable = true; # Dead code detection
            statix.enable = true; # Nix linting
            yamllint.enable = true; # YAML validation
          };
        };
      ...
      devShell =
        ...
        let
          ...
          preCommitCheck = pre-commit-check system;

          # Create shell with pre-commit integration
          shellWithPreCommit = baseShell.default.overrideAttrs (old: {
            shellHook = (old.shellHook or "") + preCommitCheck.shellHook;
            buildInputs = (old.buildInputs or [ ]) ++ preCommitCheck.enabledPackages;
          });
        in
        {
          default = perfIntegration.performanceOptimizations.mkOptimizedDevShell shellWithPreCommit;
        };
    ...
    let
      ...
      baseOutputs = {
        ...
        # Checks using modular check builders with pre-commit integration
        checks = forAllSystems (
          system:
          let
            baseChecks = checkBuilders.mkChecks system;
            preCommitCheck = pre-commit-check system;
          in
          baseChecks
          // {
