...
{ system ? null, pkgs ? null, lib ? null }:
...
let
  ...
  currentSystem =
    ...
    else if pkgs != null && pkgs ? system then pkgs.system
  ...
  platformInfo = rec {
    ...
    platform =
      if patterns.isDarwinSystem then "darwin"
      else if patterns.isLinuxSystem then "linux"
      else "unknown";

    # Architecture identification
    arch =
      if patterns.isX86_64System then "x86_64"
      else if patterns.isAarch64System then "aarch64"
      else "unknown";
  ...
  detection = {
    ...
    isDarwin = system_input:
      if system_input == currentSystem then platformInfo.isDarwin
      else builtins.match ".*-darwin" system_input != null;

    isLinux = system_input:
      if system_input == currentSystem then platformInfo.isLinux
      else builtins.match ".*-linux" system_input != null;

    isX86_64 = system_input:
      if system_input == currentSystem then platformInfo.isX86_64
      else builtins.match "x86_64-.*" system_input != null;

    isAarch64 = system_input:
      if system_input == currentSystem then platformInfo.isAarch64
      else builtins.match "aarch64-.*" system_input != null;

    # Platform string extraction
    getPlatform = system_input:
      if system_input == currentSystem then platformInfo.platform
      else if detection.isDarwin system_input then "darwin"
      else if detection.isLinux system_input then "linux"
      else errorSystem.throwValidationError "Unknown platform in system: ${system_input}";

    # Architecture string extraction
    getArch = system_input:
      if system_input == currentSystem then platformInfo.arch
      else if detection.isX86_64 system_input then "x86_64"
      else if detection.isAarch64 system_input then "aarch64"
      else errorSystem.throwValidationError "Unknown architecture in system: ${system_input}";

    # System validation
    validateSystem = system_input:
      let
        isValid = (detection.isDarwin system_input || detection.isLinux system_input) &&
  ...
  performance = {
    ...
    cacheEfficiency =
      if performance.totalQueries > 0 then
        (performance.cacheHitCount / performance.totalQueries * 100.0) else 0.0;
  };
...
in
{
  ...
  validate = {
    platform = platform: builtins.elem platform [ "darwin" "linux" ];
    arch = arch: builtins.elem arch [ "x86_64" "aarch64" ];
    system = system: builtins.elem system [
      "x86_64-darwin"
      "aarch64-darwin"
      "x86_64-linux"
      "aarch64-linux"
