...
let
  ...
  packageUtils = {
    ...
    validatePackages = packages:
      ...
      then errorSystem.throwValidationError "Invalid packages found: ${builtins.toString invalidPackages}"
      else packages;

    # Check if package exists in nixpkgs
    packageExists = packageName: nixpkgs:
      builtins.hasAttr packageName nixpkgs;

    # Get package safely with fallback
    getPackageSafe = packageName: nixpkgs: fallback:
      if packageUtils.packageExists packageName nixpkgs
      then nixpkgs.${packageName}
      else fallback;

    # Filter packages by availability
    filterAvailablePackages = packageNames: nixpkgs:
      builtins.filter (name: packageUtils.packageExists name nixpkgs) packageNames;
  };
  ...
  configUtils = {
    ...
    extractKeys = config: keys:
      actualLib.genAttrs keys (key:
        if builtins.hasAttr key config
        then config.${key}
        else errorSystem.throwConfigError "Key '${key}' not found in config"
      );

    # Validate required keys exist in config
    validateRequiredKeys = config: requiredKeys:
      let
        missingKeys = builtins.filter (key: !builtins.hasAttr key config) requiredKeys;
      in
      if builtins.length missingKeys > 0
      then errorSystem.throwValidationError "Missing required config keys: ${builtins.concatStringsSep ", " missingKeys}"
      else config;
  };
  ...
  stringUtils = {
    ...
    hasSuffix = suffix: string:
      ...
      then false
      else builtins.substring (stringLen - suffixLen) suffixLen string == suffix;

    # Remove prefix from string if present
    removePrefix = prefix: string:
      if stringUtils.hasPrefix prefix string
      then builtins.substring (builtins.stringLength prefix) (builtins.stringLength string) string
      else string;

    # Remove suffix from string if present
    removeSuffix = suffix: string:
      if stringUtils.hasSuffix suffix string
      then builtins.substring 0 (builtins.stringLength string - builtins.stringLength suffix) string
      else string;
  ...
  pathUtils = {
    ...
    joinPath = components:
      stringUtils.joinStrings "/" (builtins.filter (x: x != "" && x != null) components);
    ...
    basename = path:
      let
        parts = stringUtils.splitString "/" path;
      in
      if builtins.length parts == 0 then path
  ...
  attrUtils = {
    ...
    hasAttrPath = path: attrs:
      let
        ...
        checkPath = pathParts: currentAttrs:
    ...
    getAttrPath = path: attrs: default:
      let
        parts = if builtins.isList path then path else stringUtils.splitString "." path;
        getValue = pathParts: currentAttrs:
    ...
    setAttrPath = path: value: attrs:
      let
        parts = if builtins.isList path then path else stringUtils.splitString "." path;
        setValue = pathParts: currentAttrs:
...
in
{
  ...
  inherit systemUtils packageUtils configUtils listUtils stringUtils pathUtils attrUtils;

  # Legacy compatibility - export functions at top level
  inherit (systemUtils) isSystem isDarwin isLinux isX86_64 isAarch64;
  inherit (packageUtils) filterValidPackages mergePackageLists getPackageNames validatePackages;
  ...
  categories = [ "system" "packages" "config" "lists" "strings" "paths" "attrs" ];
}
