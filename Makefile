...
build: check-user
	...
	@OS=$$(uname -s); \
	if [ "$${OS}" = "Darwin" ]; then \
		export USER=$(USER); $(NIX) build --impure --refresh --no-link --quiet .#darwinConfigurations.$(CURRENT_SYSTEM).system $(ARGS); \
	else \
		export USER=$(USER); $(NIX) build --impure --refresh --no-link --quiet .#nixosConfigurations.$(CURRENT_SYSTEM).config.system.build.toplevel $(ARGS); \
	fi
