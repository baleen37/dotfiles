# Generated by Powerlevel10k configuration wizard on 2025-10-27 at 10:51 KST.
...
# 2 lines, dotted, lightest-ornaments, compact, concise, transient_prompt,
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    # Compact right prompt - only essential information
  ...
  # Compact Git formatter.
  #
  # Example output: main +! or main* or main>2
  #
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      (( $#branch > 15 )) && branch[8,-8]=".."
      res+="${branch//\%/%%}"
    fi

    # Show tag if not on branch
    ...
    # Show commit hash if no branch and no tag
    [[ -z $VCS_STATUS_LOCAL_BRANCH && -z $VCS_STATUS_TAG ]] &&
      res+="@${VCS_STATUS_COMMIT[1,6]}"

    # Simple status indicators
    local status=""

    # Ahead/behind
    (( VCS_STATUS_COMMITS_AHEAD )) && status+=">${VCS_STATUS_COMMITS_AHEAD}"
    (( VCS_STATUS_COMMITS_BEHIND )) && status+="<${VCS_STATUS_COMMITS_BEHIND}"

    # Working directory status (compact)
    (( VCS_STATUS_NUM_STAGED )) && status+="+"
    (( VCS_STATUS_NUM_UNSTAGED )) && status+="!"
    (( VCS_STATUS_NUM_UNTRACKED )) && status+="?"
    (( VCS_STATUS_NUM_CONFLICTED )) && status+="~"
    [[ -n $VCS_STATUS_ACTION ]] && status+="=${VCS_STATUS_ACTION}"
  ...
  ####################################[ time: current time ]####################################
  # Time disabled for compact prompt
