...
aggregator_new() {
    ...
    [[ -n "$aggregator_id" ]] || { echo "Error: aggregator_id is required"; return 1; }
...
aggregator_add_result() {
    ...
    [[ -n "$aggregator_id" ]] || { echo "Error: aggregator_id is required"; return 1; }
    [[ -n "$result_data" ]] || { echo "Error: result_data is required"; return 1; }
...
aggregator_add_suite() {
    ...
    [[ -n "$aggregator_id" ]] || { echo "Error: aggregator_id is required"; return 1; }
    [[ -n "$suite_id" ]] || { echo "Error: suite_id is required"; return 1; }
...
_aggregator_convert_suite_to_result() {
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        if [[ ${#files[@]} -gt 0 ]]; then
            test_files_json="[$(IFS=,; echo "${files[*]}")]"
        fi
    fi
