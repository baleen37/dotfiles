...
test_result_new() {
  ...
  [[ -n $result_id ]] || {
    echo "Error: result_id is required"
    return 1
  }
  [[ -n $test_name ]] || {
...
test_result_pass() {
  ...
  [[ $assertion_count =~ ^[0-9]+$ ]] || {
    echo "Error: assertion_count must be a number"
    return 1
  }
...
test_result_fail() {
  ...
  [[ -n $error_message ]] || {
    echo "Error: error_message is required"
    return 1
  }
  [[ $assertion_count =~ ^[0-9]+$ ]] || {
    echo "Error: assertion_count must be a number"
  ...
  [[ $failed_assertions =~ ^[0-9]+$ ]] || {
    echo "Error: failed_assertions must be a number"
    return 1
  }
...
test_result_skip() {
  ...
  [[ -n $skip_reason ]] || {
    echo "Error: skip_reason is required"
    return 1
  }
...
test_result_set_exit_code() {
  ...
  [[ $exit_code =~ ^[0-9]+$ ]] || {
    echo "Error: exit_code must be a number"
    return 1
  }
