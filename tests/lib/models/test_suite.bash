...
test_suite_new() {
  ...
  [[ -n $suite_name ]] || {
    echo "Error: suite_name is required"
    return 1
  }
  [[ -n $base_path ]] || {
...
test_suite_add_file() {
  ...
  [[ -n $test_file_path ]] || {
    echo "Error: test_file_path is required"
    return 1
  }
  [[ -f $test_file_path ]] || {
    echo "Error: test file '$test_file_path' does not exist"
    return 1
  }
...
test_suite_update_stats() {
  ...
  [[ $total =~ ^[0-9]+$ ]] || {
    echo "Error: total must be a number"
    return 1
  }
  [[ $passed =~ ^[0-9]+$ ]] || {
