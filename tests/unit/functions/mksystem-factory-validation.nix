...
in
{
  ...
  "unit-functions-mksystem-factory-validation" = helpers.testSuite "mksystem-factory-validation" [
    ...
    (helpers.assertTestWithDetails "mksystem-file-importability-succeeds" true
      (builtins.isFunction subject)
      "mkSystem.nix should be importable and return function"
    )

    # Function callability tests
    (helpers.assertTestWithDetails "mksystem-with-valid-inputs-returns-function" true
      (builtins.isFunction (subject testInputs.valid))
      "mkSystem should return function when called with valid inputs"
    )

    # Input validation tests
    (helpers.assertTestWithDetails "mksystem-accepts-inputs-structure" true (
    ...
    (
      ...
      in
      helpers.assertTestWithDetails "mksystem-with-invalid-inputs-handles-gracefully" true (
    ...
    (
      ...
      in
      helpers.assertTestWithDetails "mksystem-can-be-composed" true (builtins.isFunction composed)
