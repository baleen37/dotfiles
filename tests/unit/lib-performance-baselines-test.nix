# ============================================================================
# DISABLED TEST - lib-performance-baselines-test.nix
# ============================================================================
#
# WHAT THIS TESTS:
#   Unit tests for lib/performance-baselines.nix baseline configuration.
#   Tests cover:
#   - systemBaselines: Platform-specific thresholds for aarch64-darwin,
#     x86_64-linux, aarch64-linux
#   - operationBaselines: Performance baselines for config-load, module-eval,
#     build-operation, test-execution with size tiers (small/medium/large)
#   - regressionThresholds: Time and memory regression factors with
#     warning/critical levels
#   - monitoringConfig: Sample intervals, max samples, alert thresholds
#   - getCurrentBaseline: Dynamic baseline selection with fallback
#   - createBaselineMeasurements: Baseline measurement creation
#   - Legacy exports: Backward compatibility functions
#   - Testing helpers: mkPerfTest, mkBenchmarkSuite
#
# WHY DISABLED:
#   The performance baselines system is tightly coupled with the performance
#   testing framework and shares similar issues:
#   1. Baselines may be platform-specific and not portable across architectures
#   2. Threshold values may need frequent updates as codebase evolves
#   3. System-specific baselines may not apply to all test environments
#   4. May require historical performance data to be meaningful
#   5. Dependent on performance.nix framework stability
#
# WHAT WOULD NEED TO BE FIXED TO RE-ENABLE:
#   1. Establish and validate baseline values for all supported platforms
#   2. Create mechanism for updating baselines as performance characteristics change
#   3. Ensure baselines are reasonable and not overly strict
#   4. Consider making baselines configurable per environment
#   5. Verify baseline accuracy on all supported platforms (macOS, Linux x64/ARM)
...
#
# TOTAL TESTS: 20 tests covering baseline configuration and threshold validation
# ============================================================================
...
in
{
  ...
  exports-exist = helpers.assertTest "baselines-exports-exist" (
  ...
  system-baselines-has-systems = helpers.assertTest "baselines-system-baselines-has-systems" (
  ...
  aarch64-darwin-baseline-structure = helpers.assertTest "baselines-aarch64-darwin-structure" (
    let
  ...
  aarch64-darwin-build-thresholds-positive = helpers.assertTest "baselines-aarch64-darwin-build-positive" (
    let
  ...
  aarch64-darwin-memory-thresholds-positive = helpers.assertTest "baselines-aarch64-darwin-memory-positive" (
    let
  ...
  aarch64-darwin-test-thresholds-positive = helpers.assertTest "baselines-aarch64-darwin-test-positive" (
    let
  ...
  x86_64-linux-baseline-structure = helpers.assertTest "baselines-x86_64-linux-structure" (
    let
  ...
  operation-baselines-has-operations = helpers.assertTest "baselines-operation-baselines-has-operations" (
  ...
  config-load-has-sizes = helpers.assertTest "baselines-config-load-has-sizes" (
    let
  ...
  regression-thresholds-structure = helpers.assertTest "baselines-regression-thresholds-structure" (
    let
  ...
  regression-thresholds-reasonable = helpers.assertTest "baselines-regression-thresholds-reasonable" (
    let
      thresholds = baselines.regressionThresholds;
    in
  ...
  monitoring-config-structure = helpers.assertTest "baselines-monitoring-config-structure" (
    let
  ...
  monitoring-config-alerts-thresholds = helpers.assertTest "baselines-monitoring-config-alerts-thresholds" (
    let
  ...
  get-current-baseline-exists = helpers.assertTest "baselines-get-current-baseline-exists" (
  ...
  get-current-baseline-returns-baseline = helpers.assertTest "baselines-get-current-baseline-returns" (
    let
  ...
  get-current-baseline-fallback = helpers.assertTest "baselines-get-current-baseline-fallback" (
    let
  ...
  create-baseline-measurements-exists = helpers.assertTest "baselines-create-baseline-measurements-exists" (
  ...
  create-baseline-measurements-returns = helpers.assertTest "baselines-create-baseline-measurements-returns" (
    let
  ...
  legacy-exports-exist = helpers.assertTest "baselines-legacy-exports-exist" (
  ...
  testing-helpers-exist = helpers.assertTest "baselines-testing-helpers-exist" (
