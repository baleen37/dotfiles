...
pkgs.runCommand "error-handling-test"
{
  ...
  if [[ -f "${src}/lib/error-handling.nix" ]]; then
    ...
    # Test if we can import it (using instantiate instead of eval for compatibility)
    if nix-instantiate --eval --expr 'import ${src}/lib/error-handling.nix { pkgs = import <nixpkgs> {}; }' &>/dev/null; then
  ...
  if [[ -n "$errorHandlingLib" ]]; then
    # Test error creation via nix-instantiate
  ...
  unset USER
  if nix-instantiate --eval --expr 'builtins.getEnv "USER"' 2>&1 | grep -q '""'; then
  ...
  if nix-instantiate --eval --expr 'builtins.fromJSON (builtins.readFile '"$TEST_DIR"'/flake/flake.lock)' 2>&1 | grep -q "error"; then
  ...
  if nix-instantiate --eval --expr 'import ${testEnv}/lib/broken-module.nix { pkgs = import <nixpkgs> {}; }' 2>&1 | grep -q "error"; then
  ...
  if nix-instantiate --eval --expr 'import ${testEnv}/lib/module-a.nix { pkgs = import <nixpkgs> {}; }' 2>&1 | grep -q "infinite recursion"; then
  ...
  current_system=$(nix-instantiate --eval --expr 'builtins.currentSystem' | tr -d '"')
