...
# TDD for Phase 4 Sprint 4.1 - Directory structure improvement

{ pkgs, flake ? null, src ? ../. }:

pkgs.runCommand "directory-structure-optimization-test"
{
  ...
  lib_dirs=("${src}/apps/aarch64-darwin/lib" "${src}/apps/aarch64-linux/lib" "${src}/apps/x86_64-darwin/lib" "${src}/apps/x86_64-linux/lib")
  ...
  if [[ -d "${src}/scripts/lib" ]]; then
    common_lib_files=$(find "${src}/scripts/lib" -name "*.sh" | wc -l)
  ...
  if [[ -d "${src}/scripts/platform" ]]; then
    override_files=$(find "${src}/scripts/platform" -name "*-overrides.sh" | wc -l)
  ...
  backup_files=$(find "${src}/apps" -name "*.backup" 2>/dev/null | wc -l)
