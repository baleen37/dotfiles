...
let
  ...
  formatContains = module: lib.hasInfix ("$" + module) starshipSettings.format;
  ...
  assertFormatHasModule =
    module:
    helpers.assertTest "starship-format-has-${
      lib.replaceStrings [ "_" ] [ "-" ] module
    }" (formatContains module) "Starship format should include ${module} module";

  # Helper to test module is disabled
  assertModuleDisabled =
    moduleName:
    helpers.assertTest "starship-${
      lib.replaceStrings [ "_" ] [ "-" ] moduleName
    }-disabled" (isModuleDisabled moduleName) "Starship ${moduleName} module should be disabled";
...
helpers.testSuite "starship-configuration" (
  [
    ...
    (helpers.assertTest "starship-directory-repo-root-format"
      (lib.hasInfix "[$before_root_path]($before_repo_root_style)[$repo_root]($repo_root_style)" starshipSettings.directory.repo_root_format)
