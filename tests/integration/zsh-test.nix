...
let
  ...
  aliasValueMatches =
    aliasName: expectedValue: (builtins.getAttr aliasName shellAliases) == expectedValue;
  ...
  initContentHasAll = patterns: builtins.all initContentHas patterns;
...
in
{
  ...
  value = helpers.testSuite "zsh-configuration" [
    ...
    (helpers.assertTest "zsh-completion-enabled" zshSettings.enableCompletion
      "zsh completion should be enabled"
    )
    (helpers.assertTest "zsh-fast-completion" (
    ...
    (helpers.assertTest "alias-gcp-exists" (hasAlias "gcp")
      "alias 'gcp' (git cherry-pick) should exist"
    )
    ...
    (helpers.assertTest "multidot-cd-fn" (initContentHas "cd() {")
      "multi-dot cd function should be defined in initContent"
    )

    # ls color alias
    (helpers.assertTest "alias-ls-color" (hasAlias "ls") "alias 'ls' should exist with --color=auto")

    # Utility aliases
    (helpers.assertTest "alias-la-exists" (hasAlias "la") "alias 'la' (ls -la) should exist")
    (helpers.assertTest "alias-diff-difftastic" (aliasValueMatches "diff" "difft")
    ...
    (helpers.assertTest "fzf-height-option" (lib.any (opt: lib.hasInfix "--height 40%" opt) (
      fzfSettings.defaultOptions or [ ]
    )) "fzf should have height option set to 40%")

    (helpers.assertTest "fzf-layout-reverse" (lib.any (opt: lib.hasInfix "--layout=reverse" opt) (
      fzfSettings.defaultOptions or [ ]
    )) "fzf should use reverse layout")

    (helpers.assertTest "fzf-preview-bat" (lib.any (opt: lib.hasInfix "bat" opt) (
      fzfSettings.defaultOptions or [ ]
    ...
    (helpers.assertTest "history-ignore-pwd-ls-cd" (initContentHas "pwd:ls:cd")
    ...
    (helpers.assertTest "path-go-bin" (initContentHas "go/bin") "PATH should include go/bin directory")
    ...
    (helpers.assertTest "homebrew-path-macos" (
      ...
      # Skip on Linux, or check on Darwin
    ...
    (helpers.assertTest "onepassword-group-containers" (
      if isDarwin then (initContentHas "Group Containers") else true # Skip on non-Darwin platforms
    ) "_setup_1password_agent should check Group Containers on macOS")

    (helpers.assertTest "onepassword-fallback-locations" (
      if isDarwin then (initContentHas ".1password/agent.sock") else true # Skip on non-Darwin platforms
