...
let
  ...
  hasPluginByName =
    pname: vimConfigUsable && pluginHelpers.hasPluginByName vimConfig.programs.vim.plugins pname;

  # Helper function to check if extraConfig contains a pattern (with vimConfigUsable guard)
  hasConfigPattern =
    pattern:
    vimConfigUsable && pluginHelpers.hasConfigPattern vimConfig.programs.vim.extraConfig pattern;

  # Helper function to check multiple config patterns (all must match)
  hasAllConfigPatterns =
    patterns:
    vimConfigUsable && pluginHelpers.hasAllConfigPatterns vimConfig.programs.vim.extraConfig patterns;
