...
let
  ...
  allTests = [
    ...
    (helpers.assertTest "ghostty-config-usable" ghosttyConfigUsable
      "ghostty.nix should be importable and usable"
    )

    # Test that home.packages exists
    (helpers.assertTest "ghostty-has-packages" (
      ghosttyConfigUsable
      && builtins.hasAttr "home" ghosttyConfig
    ...
    (helpers.assertTest "ghostty-config-path" (
      ghosttyConfigUsable && builtins.hasAttr ".config/ghostty" ghosttyConfig.home.file
    ...
    (helpers.assertTest "ghostty-font-family-d2coding-set" (hasConfigLine "font-family.*=.*D2Coding")
    ...
    (helpers.assertTest "ghostty-config-not-empty" (
    ...
    (helpers.assertTest "ghostty-source-is-directory" (
      ghosttyConfigUsable && ghosttyConfig.home.file.".config/ghostty".recursive or false
    ) "Ghostty file source should be a directory (recursive = true)")

    (helpers.assertTest "ghostty-force-symlink" (
      ghosttyConfigUsable && ghosttyConfig.home.file.".config/ghostty".force or false
