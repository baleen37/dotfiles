...
let
  ...
  darwinMachinesList = lib.filterAttrs (_n: v: v != null) {
  ...
  nixosMachinesList = lib.filterAttrs (_n: v: v != null) {
  ...
  hasDarwinRequiredAttrs =
    config: (config ? system) && (config ? environment) && (config ? programs) && (config ? nix);

  # Helper to check for required NixOS attributes
  hasNixOSRequiredAttrs =
    config: (config ? system) && (config ? networking) && (config ? services) && (config ? nixpkgs);

  # Test helper to check for state version
  hasStateVersion = config: config.system.stateVersion or null != null;

  # Test helper to check for system packages
  hasSystemPackages = config: builtins.length (config.environment.systemPackages or [ ]) > 0;

  # Test helper to verify networking configuration (NixOS)
  hasNetworkingConfig = config: config.networking.hostName or null != null;
...
in
{
  ...
  value = helpers.testSuite "machine-builds" [
    ...
    (helpers.assertTest "darwin-machines-have-config" (builtins.all (m: m ? config) (
    ...
    (helpers.assertTest "darwin-required-attrs" (builtins.all (
      m: hasDarwinRequiredAttrs (m.config or { })
    ) (lib.attrValues darwinMachinesList)) "All Darwin machines should have required attributes")

    # Test 4: Darwin machines have state version
    (helpers.assertTest "darwin-state-version" (builtins.all (m: hasStateVersion (m.config or { })) (
      lib.attrValues darwinMachinesList
    )) "All Darwin machines should have stateVersion set")

    # Test 5: Darwin machines have system packages
    (helpers.assertTest "darwin-system-packages" (builtins.all (
      m: hasSystemPackages (m.config or { })
    ) (lib.attrValues darwinMachinesList)) "All Darwin machines should have system packages")
    ...
    (helpers.assertTest "nixos-machines-have-config" (builtins.all (m: m ? config) (
    ...
    (helpers.assertTest "nixos-required-attrs" (builtins.all (
      m: hasNixOSRequiredAttrs (m.config or { })
    ) (lib.attrValues nixosMachinesList)) "All NixOS machines should have required attributes")

    # Test 9: NixOS machines have networking config
    (helpers.assertTest "nixos-networking-config" (builtins.all (
      m: hasNetworkingConfig (m.config or { })
    ) (lib.attrValues nixosMachinesList)) "All NixOS machines should have networking configuration")

    # ===== Cross-Platform Compatibility Tests =====

    # Test 10: All machines have system attribute
    (helpers.assertTest "all-have-system" (builtins.all (m: (m.config or { }) ? system) (
    ...
    (helpers.assertTest "all-buildable" (builtins.all (m: (m.config or { }) ? system) (
      if isDarwin then
        lib.attrValues darwinMachinesList
    ...
    (helpers.assertTest "darwin-have-nix" (builtins.all (m: (m.config or { }) ? nix) (
      lib.attrValues darwinMachinesList
    )) "All Darwin machines should have nix attribute")

    # Test 13: Darwin machines have programs attribute
    (helpers.assertTest "darwin-have-programs" (builtins.all (m: (m.config or { }) ? programs) (
      lib.attrValues darwinMachinesList
    )) "All Darwin machines should have programs attribute")

    # Test 14: NixOS machines have nixpkgs attribute
    (helpers.assertTest "nixos-have-nixpkgs" (builtins.all (m: (m.config or { }) ? nixpkgs) (
      lib.attrValues nixosMachinesList
    )) "All NixOS machines should have nixpkgs attribute")

    # Test 15: NixOS machines have services attribute
    (helpers.assertTest "nixos-have-services" (builtins.all (m: (m.config or { }) ? services) (
      lib.attrValues nixosMachinesList
    ...
    (helpers.assertTest "unique-machine-names" (
      builtins.length (
        lib.unique ((lib.attrNames darwinMachinesList) ++ (lib.attrNames nixosMachinesList))
      ) == (
        (builtins.length (lib.attrNames darwinMachinesList))
    ...
    (helpers.assertTest "darwin-valid-state-version" (builtins.all (
      m: (m.config.system.stateVersion or "") != ""
    ) (lib.attrValues darwinMachinesList)) "All Darwin machines should have valid stateVersion")

    # Test 21: NixOS machines have valid configuration
    (helpers.assertTest "nixos-valid-config" (builtins.all (m: (m.config.system or "") != "") (
      lib.attrValues nixosMachinesList
    ...
    (helpers.assertTest "essential-config-structure" (builtins.all
      (
        m:
        if isDarwin then
