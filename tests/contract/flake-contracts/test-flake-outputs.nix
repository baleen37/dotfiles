...
in runTests {
  ...
  testFlakeHasRequiredOutputs = {
    expr = if flakeOutputs.success 
           then builtins.all (output: builtins.hasAttr output flakeOutputs.value) 
                ["nixosConfigurations" "darwinConfigurations" "homeConfigurations" "checks" "apps"]
           else false;
    expected = true;
  };
  ...
  testFlakeHasTestingPackages = {
    expr = if flakeOutputs.success
           then builtins.hasAttr "packages" flakeOutputs.value &&
  ...
  testFlakeHasTestingApps = {
    expr = if flakeOutputs.success
           then builtins.hasAttr "apps" flakeOutputs.value &&
                ...
                builtins.all (app: builtins.hasAttr app flakeOutputs.value.apps.x86_64-linux)
                  ["test-unit" "test-contract" "test-integration" "test-e2e" "test-coverage"]
           else false;
    expected = true;
  };
  ...
  testFlakeHasTestingChecks = {
    expr = if flakeOutputs.success
           then builtins.hasAttr "checks" flakeOutputs.value &&
                ...
                builtins.all (check: builtins.hasAttr check flakeOutputs.value.checks.x86_64-linux)
                  ["test-unit-all" "test-contract-all" "test-integration-all" "test-e2e-all"]
           else false;
    expected = true;
  };
  ...
  testTestUnitAppStructure = {
    expr = if flakeOutputs.success
           ...
           in if app != null
              then builtins.hasAttr "type" app && 
                   app.type == "app" &&
                   builtins.hasAttr "program" app
              else false
           else false;
    expected = true;
  };
  ...
  testNixUnitInputIntegration = {
    expr = if flakeOutputs.success
           ...
           in builtins.hasAttr "nix-unit" inputs
           else false;
    expected = true;
  };
  ...
  testNamakaInputIntegration = {
    expr = if flakeOutputs.success
           then let
             inputs = flakeOutputs.value.inputs or {};
           in builtins.hasAttr "namaka" inputs
           else false;
    expected = true;
  };
  ...
  testFlakeCheckerInputIntegration = {
    expr = if flakeOutputs.success
           then let
             inputs = flakeOutputs.value.inputs or {};
           in builtins.hasAttr "flake-checker" inputs
           else false;
    expected = true;
  };
  ...
  testTestingDevShellExists = {
    expr = if flakeOutputs.success
           then builtins.hasAttr "devShells" flakeOutputs.value &&
  ...
  testDefaultDevShellHasTestingTools = {
    expr = if flakeOutputs.success
           ...
           in if defaultShell != null
              then builtins.hasAttr "nativeBuildInputs" defaultShell &&
  ...
  testTestingOverlayExists = {
    expr = if flakeOutputs.success
           then builtins.hasAttr "overlays" flakeOutputs.value &&
  ...
  testTestingOverlayProvidesPackages = {
    expr = if flakeOutputs.success
           ...
           in if testingOverlay != null
              then let
                pkgs = import <nixpkgs> { overlays = [ testingOverlay ]; };
              in builtins.hasAttr "test-runner" pkgs &&
                 builtins.hasAttr "coverage-tools" pkgs
              else false
           else false;
    expected = true;
  };
  ...
  testFormatterSupportsTestFiles = {
    expr = if flakeOutputs.success
           then builtins.hasAttr "formatter" flakeOutputs.value &&
  ...
  testLibExportsTestingFunctions = {
    expr = if flakeOutputs.success
           then builtins.hasAttr "lib" flakeOutputs.value &&
                builtins.hasAttr "testing" flakeOutputs.value.lib
           else false;
    expected = true;
  };
  ...
  testLibTestingHasTestBuilders = {
    expr = if flakeOutputs.success
           ...
           in builtins.hasAttr "mkTestSuite" testing &&
              builtins.hasAttr "mkCoverageReport" testing
           else false;
    expected = true;
  };
  ...
  testTestingTemplateExists = {
    expr = if flakeOutputs.success
           then builtins.hasAttr "templates" flakeOutputs.value &&
  ...
  testTestingTemplateStructure = {
    expr = if flakeOutputs.success
           ...
           in if template != null
              then builtins.hasAttr "path" template &&
                   builtins.hasAttr "description" template
              else false
           else false;
    expected = true;
  };
  ...
  testFlakeHasCorrectDescription = {
    expr = if flakeOutputs.success
           ...
           in builtins.hasAttr "description" flake &&
              builtins.isString flake.description
           else false;
    expected = true;
  };
  ...
  testFlakeInputsAreValid = {
    expr = if flakeOutputs.success
           then let
             flake = flakeOutputs.value;
           in builtins.hasAttr "inputs" flake &&
              builtins.isAttrs flake.inputs
           else false;
    expected = true;
  };
    ...
    expr = if flakeOutputs.success
           then let
             ...
             requiredOutputs = [
               ...
               "checks" "apps" "packages" "devShells" "lib"
             ];
           in builtins.all (output: builtins.hasAttr output outputs) requiredOutputs
           else false;
    expected = true;
  };
}